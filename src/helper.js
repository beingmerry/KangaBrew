      ...
      .catch(error => {
        ...
        if (webStorageMode) {
          ...
          if (getLocalBreweries !== '') {
            localFavoriteBreweries = JSON.parse(
              localStorage.getItem('localFavoriteBreweries')
            )
...
const renderFavorite = brewery => {
  ...
  breweryDeleteBtn.addEventListener('click', () => {
    if (serverStateGood) {
      ...
      favorites.splice(favorites.indexOf(brewery.name), 1)
      localFavoriteBreweries = localFavoriteBreweries.filter(
        el => el.name !== brewery.name
      )
      debugger
      ...
      localStorage.setItem(
        'localFavoriteBreweries',
        JSON.stringify(localFavoriteBreweries)
      )
    } else {
      favorites.splice(favorites.indexOf(brewery.name), 1)
      myBreweriesList.removeChild(newBrewery)
...
const renderBreweryRow = brewery => {
  ...
  if (brewery.phone !== null) {
    phoneNumberCheck = `(${brewery.phone.slice(0, 3)}) ${brewery.phone.slice(
      3,
      6
  ...
  const addButton = document.querySelector(
    `#quick-add-brewery-${currentResultNumber}`
  )
  const detailButton = document.querySelector(
    `#see-brewery-details-${currentResultNumber}`
  )
...
const postNewBrewery = (newBrewery = currentBrewery) => {
  ...
  if (0 === favorites.filter(favorite => favorite === newBrewery.name).length) {
    if (serverStateGood) {
      fetch(`http://localhost:3000/myBreweryList`, {
        method: `POST`,
        headers: {
      ...
      if (webStorageMode) {
        ...
        console.log(
          'No server at http://localhost:3000, rendering locally only...'
        )
        renderFavorite(newBrewery)
      }
    }
